...
if __name__ == '__main__':
    ...
    for filename in filter(lambda x: 'gitkeep' not in x and not Path(x).is_file(), os.listdir(Path(r"C:\Users\almogsh\PycharmProjects\Py_ChordAR\photos\asd\almog"))):
        try:
            guitar = GuitarImage(
            img_path=Path(rf"C:\Users\almogsh\PycharmProjects\Py_ChordAR\photos\asd\almog\{filename}"))  # , file_name=filename)
